...
export class PineconeService {
     ...
     */
    async processPDFContent(content, title, category, chunkSize = 1000, overlap = 200) {
        if (!this.index) {
    ...
    async getEmbeddingsForChunks(chunks) {
        ...
        for (let i = 0; i < chunks.length; i += batchSize) {
            ...
            const batchEmbeddings = await Promise.all(batch.map(chunk => this.getEmbedding(chunk)));
