...
import { authService, authenticateToken, isAdminAuthorized } from './middleware/auth.middleware.js';
import { validateLogin, validateCreateStudent, validateCreateEcosSession, validateEcosMessage, validateEcosEvaluation, validateRequestSize, validateContentType } from './middleware/validation.middleware.js';
import { authRateLimit, apiRateLimit, strictRateLimit, ecosSessionRateLimit } from './middleware/rate-limit.middleware.js';
export async function registerRoutes(app) {
    ...
    app.post("/api/admin/sync-scenarios", async (req, res) => {
    ...
    app.post("/api/ecos/sessions/:sessionId/evaluate", strictRateLimit.middleware(), validateContentType(), validateRequestSize(), validateEcosEvaluation, async (req, res) => {
        const { email } = req.query;
        if (!email || !isAdminAuthorized(email)) {
            return res.status(403).json({ message: "Accès non autorisé" });
        }
